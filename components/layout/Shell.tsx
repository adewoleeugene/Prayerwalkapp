...
import {
  LayoutDashboard, Settings, LogOut, Bell, ChevronRight,
  User, ShieldCheck, Zap, Menu, X,
} from 'lucide-react';
...
export default function Shell({ children }: ShellProps) {
  ...
  const role = typeof window !== 'undefined' ? localStorage.getItem('adminUserRole') : null;
  const branch = typeof window !== 'undefined' ? localStorage.getItem('adminUserBranch') : null;
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 font-bold text-sm">
                  <item.icon className={cn('h-4 w-4 transition-colors', isActive ? 'text-primary-foreground' : 'text-slate-400 group-hover:text-slate-900')} />
                  {item.label}
                </div>
        ...
        <div className="p-6 mt-auto">
          {role !== 'superadmin' && (
            <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 mb-6 relative overflow-hidden group">
              ...
              <Button variant="ghost" className="w-full justify-start gap-2 h-9 px-3 text-xs font-black text-slate-600 hover:text-destructive hover:bg-destructive/5 rounded-lg transition-all" onClick={handleLogout}>
          ...
          {role === 'superadmin' && (
            <div className="mb-6">
              <Button variant="outline" className="w-full justify-start gap-2 h-11 px-4 text-xs font-black text-slate-600 hover:text-destructive hover:bg-destructive/5 border-slate-200 rounded-xl transition-all" onClick={handleLogout}>
                <LogOut className="h-4 w-4" />
                Logout Session
              </Button>
            </div>
          )}
          <p className="text-center text-[9px] font-black text-slate-300 uppercase tracking-widest">Kharis Foundation © 2026</p>
        </div>
      </aside>
          ...
          <div className="fixed inset-0 z-[100] lg:hidden">
            ...
            <aside className="absolute top-0 left-0 w-72 h-full bg-white animate-in slide-in-from-left duration-300">
              ...
              <nav className="px-4 space-y-2 mt-4">
                {navItems.map((item) => (
                  <Button key={item.path} variant="ghost" className="w-full justify-start gap-3 h-12 px-4 rounded-xl font-bold" onClick={() => { router.push(item.path); setMobileMenuOpen(false); }}>
