...
export async function executeIdempotentRequest(params: {
  ...
  if (!idempotencyKey) {
    ...
    return { ...result, acceptedAt, idempotentReplay: false };
  }
  ...
  const existing = await prisma.$queryRawUnsafe<Array<{
    idempotency_key: string; user_id: string; endpoint: string;
    request_hash: string; response_code: number; response_json: unknown; created_at: Date;
  }>>(
    `SELECT idempotency_key, user_id, endpoint, request_hash, response_code, response_json, created_at
     FROM mobile_sync_requests WHERE idempotency_key = $1 LIMIT 1`,
    idempotencyKey
  );
  ...
  try {
    await prisma.$executeRawUnsafe(
      ...
      idempotencyKey, userId, endpoint, requestHash, executed.statusCode, JSON.stringify(executed.body)
    );
  } catch (error: any) {
    if (error?.code !== '23505') throw error;
    const raced = await prisma.$queryRawUnsafe<Array<{
      response_code: number; response_json: unknown; created_at: Date;
      user_id: string; endpoint: string; request_hash: string;
    }>>(
  ...
  return { ...executed, acceptedAt, idempotentReplay: false };
}
