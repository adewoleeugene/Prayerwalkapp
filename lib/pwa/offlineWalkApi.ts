import { pwaApi } from './apiClient';
import { createClientRequestId, enqueueAction, resolveSessionId } from './offlineQueue';
...
function localWithinRange(
  targetLocation: unknown,
  latitude: number,
  longitude: number
): { withinRange: boolean; distance: number } {
  const loc = targetLocation as Record<string, unknown> | null;
  const raw = (loc?.location as Record<string, unknown>)?.coordinates ?? loc?.location;
  ...
  if (Array.isArray(raw) && raw.length >= 2) {
    ...
    if (r.latitude !== undefined && r.longitude !== undefined) {
      targetLat = Number(r.latitude);
      targetLng = Number(r.longitude);
    }
  }
  ...
  const distance = R * c;
  const radiusMeters = Number((loc as Record<string, unknown>)?.radiusMeters ?? 50);
...
export async function startWalkOnlineOrQueue(input: {
  ...
  try {
    const res = await pwaApi.walks.start(
      input.locationId,
      input.latitude,
      input.longitude,
      input.branch,
...
export async function trackWalkOnlineOrQueue(input: {
  ...
  if (resolvedSessionId) {
    try {
      const res = await pwaApi.walks.track(
        resolvedSessionId,
        input.latitude,
        input.longitude,
        input.speed,
  ...
  return {
    data: { success: true, queued: true, acceptedAt: clientEventAt, idempotentReplay: false, status: 'queued' },
  };
}
...
export async function arriveOnlineOrQueue(input: {
  ...
  if (resolvedSessionId) {
    try {
      const res = await pwaApi.walks.arrive(
        resolvedSessionId,
        input.locationId,
        input.latitude,
        input.longitude,
...
export async function completeWalkOnlineOrQueue(input: {
  ...
  if (resolvedSessionId) {
    try {
      const res = await pwaApi.walks.complete(
        resolvedSessionId,
        input.locationId,
        input.latitude,
        input.longitude,
