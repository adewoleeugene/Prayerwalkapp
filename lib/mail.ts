...
export async function sendEmail(params: { to: string; subject: string; html: string }): Promise<void> {
  ...
  if (!transporter) {
    console.warn('GMAIL_USER or GMAIL_APP_PASSWORD not set. Email delivery skipped.', { to: params.to, subject: params.subject });
    return;
  }
  await transporter.sendMail({ from: fromAddress, to: params.to, subject: params.subject, html: params.html });
}
